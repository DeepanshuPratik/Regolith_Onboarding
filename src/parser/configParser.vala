 ...
 */
 ...
 public class Keybinding {
     ...
     public Keybinding (string label, string spec,string id) {
         this.id = id;
         this.label = label;
         this.spec = spec;
     }
 ...
 public class ConfigParser {
     private const string ONBOARDING_LINE_WRAPPER = "##";
     private const string ONBOARDING_PARAM_DELIMITER = "//";
     private const int PARAMETER_COUNT = 4;
     private const int MIN_LINE_LENGTH = 16; // "##w//x//y//z//##".length
     ...
     public Map<string, ArrayList<Keybinding> > parse () throws PARSE_ERROR, GLib.Error {
         ...
         var prefix = ONBOARDING_LINE_WRAPPER;
         if (line_prefix != "") {
             prefix = line_prefix + ONBOARDING_LINE_WRAPPER;
         }
     ...
     private bool lineMatch (string line, string prefix) {
         ...
         return line.length > MIN_LINE_LENGTH &&
                ...
                line.substring (ONBOARDING_LINE_WRAPPER.length + 1).contains (ONBOARDING_LINE_WRAPPER) &&
                line.contains (ONBOARDING_PARAM_DELIMITER);
     }
 
     /**
      * ## category // action // keybinding // id = category_sequenceNo ## anything else
      */
     private Keybinding parseLine (string line, Map<string, ArrayList<Keybinding> > configMap) throws PARSE_ERROR.BAD_PARAM_MATCH {
         ...
         string valueList = line.substring (ONBOARDING_LINE_WRAPPER.length, termSequenceIndex - ONBOARDING_LINE_WRAPPER.length);
         // Tokenize parameters
         string[] values = valueList.split (ONBOARDING_PARAM_DELIMITER);
